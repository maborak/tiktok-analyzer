...
# ─── Load .env from project root (script lives in scripts/) ──────────────────
# Variables already set in the environment (or via -e flags later) take priority.
...
if [[ -f "$ENV_FILE" ]]; then
    ...
    # Read each non-blank, non-comment line as KEY=VALUE
    while IFS= read -r line || [[ -n "$line" ]]; do
        ...
        [[ -z "$line" ]] && continue
        # Only export if the var is not already set
        key="${line%%=*}"
        if [[ -z "${!key+x}" ]]; then
...
IMAGE_NAME=""
BUILD_MODE="runtime"
declare -a BUILD_ARGS=()  # Array to store custom build args from -e flags
...
OPTIONAL ARGUMENTS:
  ...
  --build-mode=<mode>       Build mode: "runtime" or "static"
                             Default: "runtime"
                             - runtime: Build at container startup (allows .env changes)
                             - static: Multi-stage build, export dist folder only

  -e KEY=VALUE              Pass environment variable as build arg
  --env KEY=VALUE            (can be used multiple times)
                             Example: -e VITE_API_BASE_URL=http://api:8000
...
EXAMPLES:
  ...
  # Build in static mode (multi-stage, dist only)
  $0 --arch=amd64 --build-mode=static

  # Build with custom environment variables
  $0 --arch=amd64 -e VITE_API_BASE_URL=http://api:8000 -e VITE_DEBUG_MODE=true

BUILD MODES:
  - runtime (default): Includes full source, builds at container startup from .env
  - static: Multi-stage build, only includes dist folder (previous build way)
...
BUILDER MANAGEMENT:
  The script uses a persistent Docker buildx builder named "framework-ui-builder"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --image-name=*)
            ...
            ;;
        --build-mode=*)
            BUILD_MODE="${1#--build-mode=}"
            shift
            ;;
        -e|--env)
            # Handle -e KEY=VALUE or --env KEY=VALUE format
            if [[ -n "$2" ]] && [[ "$2" == *"="* ]]; then
                # Next argument is KEY=VALUE
                BUILD_ARGS+=("$2")
                shift
            else
                echo "Error: -e/--env requires KEY=VALUE format"
                echo "Example: -e VITE_API_BASE_URL=http://api:8000"
                exit 1
            fi
            shift
            ;;
        -e=*|--env=*)
            ...
            ;;
        -e*)
            # Handle -eKEY=VALUE format (no space, no equals after -e)
            # Extract everything after -e
            BUILD_ARGS+=("${1#-e}")
            shift
            ;;
        *)
...
if [[ -z "$IMAGE_NAME" ]]; then
    IMAGE_NAME="maborak/framework-ui"
fi
...
# Validate build mode
if [[ "$BUILD_MODE" != "runtime" && "$BUILD_MODE" != "static" ]]; then
    echo "Error: --build-mode must be 'runtime' or 'static'"
    exit 1
fi

# Set Dockerfile based on build mode
if [[ "$BUILD_MODE" == "static" ]]; then
    DOCKERFILE="Dockerfile.static"
    echo "Using static build mode (multi-stage, dist only)"
else
    DOCKERFILE="Dockerfile"
    echo "Using runtime build mode (build at container startup)"
fi
echo ""

# Prepare custom build args from -e flags (process early for dry-run)
CUSTOM_BUILD_ARGS=""
if [[ ${#BUILD_ARGS[@]} -gt 0 ]]; then
    for arg in "${BUILD_ARGS[@]}"; do
        # Split KEY=VALUE and create --build-arg KEY=VALUE
        if [[ "$arg" == *"="* ]]; then
            CUSTOM_BUILD_ARGS="${CUSTOM_BUILD_ARGS} --build-arg ${arg}"
        else
            echo "Warning: Skipping invalid build arg format: $arg (expected KEY=VALUE)"
        fi
    done
...
# Get version from package.json or environment variable, fallback to VERSION or "1.0.0"
if [[ -f "package.json" ]]; then
    # Try to extract version from package.json using node or grep
    if command -v node &> /dev/null; then
        APP_VERSION=$(node -p "require('./package.json').version" 2>/dev/null || echo "")
    else
        APP_VERSION=$(grep -oE '"version"[[:space:]]*:[[:space:]]*"[^"]*"' package.json | sed 's/"version"[[:space:]]*:[[:space:]]*"\([^"]*\)"/\1/' | head -1)
    fi
    # If version is 0.0.0 or empty, use VITE_APP_VERSION or fallback
    if [[ -z "$APP_VERSION" ]] || [[ "$APP_VERSION" == "0.0.0" ]]; then
        APP_VERSION=${VITE_APP_VERSION:-${VERSION:-"1.0.0"}}
    fi
else
    APP_VERSION=${VITE_APP_VERSION:-${VERSION:-"1.0.0"}}
fi

# Use VERSION for Docker tag, but APP_VERSION for the app itself
# If VERSION is still "latest" and we have a real version, use it
if [[ "$VERSION" == "latest" ]] && [[ "$APP_VERSION" != "1.0.0" ]] && [[ "$APP_VERSION" != "0.0.0" ]]; then
    VERSION="$APP_VERSION"
fi
...
display_dry_run() {
    ...
    echo "  App Version (VITE_APP_VERSION): ${APP_VERSION}"
...
BUILDER_NAME="framework-ui-builder"
...
echo "Command: docker buildx build --platform ${PLATFORMS} --build-arg BUILD_DATE=${BUILD_DATE} --build-arg VERSION=${VERSION} --build-arg VCS_REF=${VCS_REF} --build-arg GIT_COMMIT=${GIT_COMMIT} --build-arg BUILD_NUMBER=${BUILD_NUMBER} --build-arg BUILD_URL=${BUILD_URL} --build-arg VITE_APP_VERSION=${APP_VERSION} --build-arg VITE_BUILD_DATE=${BUILD_DATE} --build-arg VITE_DOCKER_TAG=${VERSION} --build-arg VITE_APP_MODE=${VITE_APP_MODE:-full} --build-arg VITE_ADMIN_ROUTE_PREFIX=${VITE_ADMIN_ROUTE_PREFIX} --build-arg VITE_USER_ROUTE_PREFIX=${VITE_USER_ROUTE_PREFIX}${CUSTOM_BUILD_ARGS} ${TAGS} ${PUSH_FLAG} ${NO_CACHE_FLAG} ${PROGRESS_FLAG} -f ${DOCKERFILE} ."
echo ""
...
docker buildx build --platform ${PLATFORMS} \
    ...
    --build-arg VITE_DOCKER_TAG=${VERSION} \
    --build-arg VITE_API_CACHE_TTL=${VITE_API_CACHE_TTL} \
    --build-arg VITE_LOADER_TYPE=${VITE_LOADER_TYPE} \
    --build-arg VITE_DEFAULT_COUNTRY=${VITE_DEFAULT_COUNTRY} \
    --build-arg VITE_DEFAULT_COUNTRY_DATA=${VITE_DEFAULT_COUNTRY_DATA} \
    --build-arg VITE_CAPTCHA_PROVIDER=${VITE_CAPTCHA_PROVIDER} \
    --build-arg VITE_RECAPTCHA_SITE_KEY=${VITE_RECAPTCHA_SITE_KEY} \
    --build-arg VITE_TURNSTILE_SITE_KEY=${VITE_TURNSTILE_SITE_KEY} \
    --build-arg VITE_APP_MODE=${VITE_APP_MODE:-full} \
    --build-arg VITE_ADMIN_ROUTE_PREFIX=${VITE_ADMIN_ROUTE_PREFIX} \
    --build-arg VITE_USER_ROUTE_PREFIX=${VITE_USER_ROUTE_PREFIX} \
    ${CUSTOM_BUILD_ARGS} \
    ${TAGS} \
    ${PUSH_FLAG} \
    ${NO_CACHE_FLAG} \
