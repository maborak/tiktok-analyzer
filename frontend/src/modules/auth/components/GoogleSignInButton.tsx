import { useState, useCallback } from 'react';
import { useGoogleLogin } from '@react-oauth/google';
import { useAuth } from '@/contexts/AuthContext';
import { googleConfig } from '@/config/env';
import toast from 'react-hot-toast';
import { Loader2 } from 'lucide-react';

interface GoogleSignInButtonProps {
  onSuccess?: () => void;
}

/**
 * Public entry point for the Google Sign-In button.
 *
 * Returns null when Google OAuth is not configured so that useGoogleLogin
 * (which requires GoogleOAuthProvider) is never called outside its provider.
 */
export function GoogleSignInButton({ onSuccess }: GoogleSignInButtonProps) {
  if (!googleConfig.enabled) return null;
  return <GoogleSignInButtonInner onSuccess={onSuccess} />;
}

/**
 * Custom-styled Google Sign-In button using useGoogleLogin hook.
 *
 * Uses the implicit flow which returns an access_token. The backend verifies
 * this via Google's userinfo endpoint. The button is styled to match our
 * design system exactly — no Google iframe.
 */
function GoogleSignInButtonInner({ onSuccess }: GoogleSignInButtonProps) {
  const { googleLogin } = useAuth();
  const [isLoading, setIsLoading] = useState(false);

  const handleSuccess = useCallback(async (tokenResponse: { access_token: string }) => {
    setIsLoading(true);
    try {
      await googleLogin(tokenResponse.access_token);
      onSuccess?.();
    } catch (error: any) {
      toast.error(error.message || 'Error signing in with Google');
    } finally {
      setIsLoading(false);
    }
  }, [googleLogin, onSuccess]);

  const login = useGoogleLogin({
    onSuccess: handleSuccess,
    onError: useCallback(() => {
      toast.error('Google sign-in was cancelled or failed');
    }, []),
    flow: 'implicit',
  });

  return (
    <button
      type="button"
      onClick={() => login()}
      disabled={isLoading}
      className="w-full flex items-center justify-center gap-3 rounded-sm border px-4 py-2.5 text-sm font-medium shadow-sm transition-all hover:opacity-80 hover:shadow focus-visible:outline-2 focus-visible:outline-offset-2 disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer"
      style={{ borderColor: 'var(--color-border-primary)', backgroundColor: 'var(--color-surface-primary)', color: 'var(--color-text-secondary)', fontFamily: 'var(--font-mono-display)' }}
    >
      {isLoading ? (
        <Loader2 className="h-5 w-5 animate-spin text-gray-500" />
      ) : (
        <svg className="h-5 w-5 flex-shrink-0" viewBox="0 0 24 24" aria-hidden="true">
          <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4" />
          <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
          <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
          <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
        </svg>
      )}
      <span>{isLoading ? 'Signing in...' : 'Continue with Google'}</span>
    </button>
  );
}
