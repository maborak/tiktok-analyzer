 ...
 *  shapes here — the user-page normalizer picks the right branch:
 *
...
/** Authenticated user-scoped client for the /tiktok/* lives view.
 *
...
export const userTikTokApiBase = {
...
// ── User-specific extensions (credit-gated CRUD) ───────────────────
//
// Beyond the read-only mirror of publicTiktokApi above, the user
// surface also supports:
//   - GET  /tiktok/credits                — balance
//   - POST /tiktok/lives                  — add monitor (1 credit)
//   - DELETE /tiktok/lives/{handle}       — remove (refund <24h)
//   - GET  /tiktok/lives/bundle           — rich card payload
//
// These don't exist on the public namespace because the public surface
// is read-only / unauthenticated.

const BASE_USER = '/tiktok';

export interface UserAddMonitorResponse {
  sub: TikTokSubscription;
  credit_debited: boolean;
}

export interface UserRemoveMonitorResponse {
  deleted: boolean;
  refunded: boolean;
}

export interface UserLivesBundle {
...
export const userTikTokApi = {
  ...
  /** Current credit balance. Drives the "You have N credits" UI prompt. */
  async getCredits(): Promise<{ balance: number }> {
    return apiRequest({ method: 'GET', url: `${BASE_USER}/credits` });
  },

  /** Rich card-grid payload mirroring admin's /lives/bundle shape. */
  async getLivesBundle(opts?: { tz?: string }): Promise<UserLivesBundle> {
    return apiRequest({
      method: 'GET',
      url: `${BASE_USER}/lives/bundle`,
      params: opts?.tz ? { tz: opts.tz } : undefined,
    });
  },

  /** Authenticated user's monitored handles (subscriptions, not the
   *  rich card payload). The shared TikTokLiveDetail component uses
   *  `listLives` from the base, which returns the public bundle
   *  shape; this method is for the user's own dashboard index. */
  async listMyLives(): Promise<TikTokSubscription[]> {
    return apiRequest({ method: 'GET', url: `${BASE_USER}/lives` });
  },

  /** Add a TikTok handle to the user's monitored list. Costs 1 credit.
   *  Throws on:
   *    - 402: insufficient credits
   *    - 409: handle already monitored by another user
   *  The caller should inspect `err.response.status` to render the
   *  right message. */
  async addMonitor(
    username: string,
    profile?: Record<string, unknown> | null,
  ): Promise<UserAddMonitorResponse> {
    return apiRequest({
      method: 'POST',
      url: `${BASE_USER}/lives`,
  ...
  /** Remove one of the user's monitors. Refunds 1 credit if the
   *  add happened within the last 24 hours. */
  async removeMonitor(handle: string): Promise<UserRemoveMonitorResponse> {
    return apiRequest({
      method: 'DELETE',
      url: `${BASE_USER}/lives/${encodeURIComponent(handle)}`,
    });
  },
};

export type UserTikTokApi = typeof userTikTokApi;
